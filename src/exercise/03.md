...
- Babel is the tool used to compile JSX syntax down to React createElement calls
- Babel can be sourced via a `<script>` tag if needed and compile a
  `<script type="text/babel">`
- Variables used in object initialization in JavaScript can be used as
  [shorthand syntax](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Object_initializer#property_definitions)
  to explicit key/value (i.e `const props = {className: 'foo'}` vs
  `const className = 'foo'; const props = {className}`)
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=03%3A%20Using%20JSX&em=imcole%40pm.me).
